...
function sessionMaxAge(session) {
  return session?.remember === false ? undefined : REMEMBER_MAX_AGE;
}
...
export function setSessionCookie(res, session) {
  ...
  appendCookie(res, `${COOKIE}=${value}; ${cookieFlags(sessionMaxAge(session))}`);
}
...
function accessFingerprint(accessCode) {
  return crypto
    .createHash("sha256")
    .update(accessCode)
    .digest("base64url");
...
export function setAccessCookie(res, accessCode) {
  if (!accessCode) return;
  const value = seal({ fp: accessFingerprint(accessCode) });
  appendCookie(res, `${ACCESS_COOKIE}=${value}; ${cookieFlags(ACCESS_MAX_AGE)}`);
}
...
export function hasAccessCookie(req, accessCode) {
  if (!accessCode) return false;
  const cookies = parseCookies(req.headers.cookie);
  const data = unseal(cookies[ACCESS_COOKIE]);
  return data?.fp === accessFingerprint(accessCode);
}

export { COOKIE, ACCESS_COOKIE };
