...
const OAUTH_ACCOUNT_COOKIE = "pb_oauth_account";
...
export function setOAuthAccountCookie(res, accountId) {
  appendCookie(
    res,
    `${OAUTH_ACCOUNT_COOKIE}=${encodeURIComponent(accountId)}; ${cookieFlags(600)}`
  );
}

export function readOAuthAccountCookie(req) {
  const cookies = parseCookies(req.headers.cookie);
  const value = cookies[OAUTH_ACCOUNT_COOKIE];
  return value ? decodeURIComponent(value) : null;
}

export function clearOAuthAccountCookie(res) {
  appendCookie(res, `${OAUTH_ACCOUNT_COOKIE}=; ${cookieFlags(0)}`);
}
...
export { COOKIE, STATE_COOKIE, OAUTH_ACCOUNT_COOKIE };
