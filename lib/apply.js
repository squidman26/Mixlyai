...
import { matchRow, formatTrack, notFoundReason } from "./match.js";
...
async function matchRows(session, rows, { includeAmbiguous }) {
  const matched = [];
  const skipped = [];
  let s = session;

  for (const row of rows) {
    const result = await matchRow(s, row.artist, row.title);
    s = result.session;

    if (result.status === "not_found") {
      skipped.push({
        ...row,
        reason: notFoundReason(session.provider),
      });
      continue;
    }
...
export async function applyPlan(session, plan, opts) {
  ...
  if (plan.action === "edit") {
    ...
    const playlistId = parsePlaylistId(playlistRef, session.provider);
    ...
    return {
      session: s4,
      summary,
      playlistUrl: playlistUrl(updated),
