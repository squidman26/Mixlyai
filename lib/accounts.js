...
import { FREE_TIER_STARTING_CREDITS, recordInitialAccountCredits } from "./credits.js";
import { hashPassword, verifyPassword } from "./password.js";

const APP_PROVIDER = "app";

const ACCOUNT_COLUMNS =
  "id, provider, external_id, username, display_name, email, email_verified_at, credits, tier, unlimited_credits, last_login_at, updated_at";
...
export async function getAccountByEmail(email) {
  if (!email?.trim() || !isSupabaseConfigured()) return null;
...
export async function markEmailVerified(accountId) {
  if (!accountId || !isSupabaseConfigured()) return null;
...
export async function authenticateAppAccount({ login, password }) {
  ...
  if (!isEmailVerified(account)) {
    throw new EmailNotVerifiedError(account.email);
  }
...
export async function savePlaylist(accountId, plan, options = {}) {
  if (!isSupabaseConfigured() || !accountId || !plan?.playlist?.name) return null;

  const supabase = getSupabaseAdmin();
  const slug = crypto.randomUUID();
  const tracks = options.tracksJson ?? plan.tracks ?? [];
  ...
  const payload = {
    ...
    external_playlist_id: options.externalPlaylistId ?? null,
    external_playlist_url: options.externalPlaylistUrl ?? null,
    updated_at: new Date().toISOString(),
  };
  ...
  const { data, error } = await supabase
    ...
    .insert(payload)
    .select(
      "playlist_slug, name, description, track_count, tracks_json, provider, external_playlist_id, external_playlist_url, updated_at"
    )
    .single();
...
export async function listSavedPlaylists(accountId) {
  ...
  return (data ?? []).map((row) => ({
    ...
    externalPlaylistUrl: row.external_playlist_url ?? null,
