...
const ACCOUNT_COLUMNS =
  "id, username, display_name, email, credits, tier, unlimited_credits, last_login_at, updated_at";
...
export async function createAppAccount({ email, username, password }) {
  ...
  if (insertError) {
    ...
    if (/password_hash|schema cache/i.test(insertError.message ?? "")) {
      throw new Error(
        "Database auth columns are missing. Run supabase/migrations/20250607150000_app_auth_and_connections.sql in the Supabase SQL editor."
      );
    }
