...
export function getTierRank(tierId) {
  return TIER_RANK[tierId] ?? 0;
}

export function canUpgradeToTier(currentTierId, targetTierId) {
  if (!isPaidTierId(targetTierId)) return false;
  return getTierRank(targetTierId) > getTierRank(currentTierId || "free");
}

export function getPlanLabel(account, user) {
  if (isUnlimitedAccount(user, account)) return "Unlimited";
...
export function buildCreditStatus(account, user) {
  ...
  return {
    ...
    tiers: Object.values(TIERS).map(({ id, name, credits, priceLabel }) => ({
      ...
      credits,
      priceLabel,
      selectable: canUpgradeToTier(account?.tier || "free", id),
    })),
  };
}
...
export async function applyTierUpgrade(accountId, tierId, paymentMeta = {}) {
  ...
  if (!canUpgradeToTier(previous?.tier || "free", tier.id)) {
...
export async function countPendingPurchases(accountId, tierId = null) {
  const supabase = getSupabaseAdmin();
  let query = supabase
    .from("credit_purchases")
  ...
  if (error) {
    throw new Error(`Failed to count pending purchases: ${error.message}`);
  }
...
export async function syncPendingPurchasesForAccount(accountId, tierId = null) {
  ...
  const { resolveCompletedPaymentForPending } = await import("./square.js");
  let applied = false;

  for (const pending of pendingRows ?? []) {
    const { payment, orderId } = await resolveCompletedPaymentForPending(pending);
    if (!payment?.id || !orderId) continue;
  ...
  const account = await getAccountCredits(accountId);
  const pendingCount = await countPendingPurchases(accountId, tierId);

  return { account, applied, pendingCount };
}
