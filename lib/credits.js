...
const ACCOUNT_CREDIT_COLUMNS =
  "id, provider, external_id, display_name, email, credits, tier, unlimited_credits";

function parseUnlimitedUserIds() {
  return (process.env.UNLIMITED_CREDITS_USER_IDS || "")
    .split(",")
    .map((value) => value.trim())
    .filter(Boolean);
}

export function isUnlimitedAccount(user, account) {
  if (account?.unlimited_credits) return true;

  const provider = user?.provider || account?.provider;
  const externalId = user?.id || account?.external_id;
  const composite = provider && externalId ? `${provider}:${externalId}` : null;

  if (composite && parseUnlimitedUserIds().includes(composite)) {
    return true;
  }

  if (externalId && parseUnlimitedUserIds().includes(externalId)) {
    return true;
  }
...
export async function deductCredits(accountId, amount, user) {
  ...
  await recordCreditTransaction({
    ...
    amount: -amount,
    balanceAfter: data.credits,
    reason: amount === CREDIT_COSTS.chatMessage ? "chat_message" : "apply_playlist",
    metadata: { tier: data.tier },
  });
