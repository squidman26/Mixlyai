...
const ACCOUNT_CREDIT_COLUMNS =
  "id, spotify_id, display_name, email, credits, tier, unlimited_credits";

function parseUnlimitedSpotifyIds() {
  return (process.env.UNLIMITED_CREDITS_SPOTIFY_IDS || "")
    .split(",")
...
export function isUnlimitedAccount(user, account) {
  ...
  const spotifyId = user?.id || account?.spotify_id;
  if (spotifyId && parseUnlimitedSpotifyIds().includes(spotifyId)) {
    return true;
  }
...
export async function applyTierUpgrade(accountId, tierId) {
...
export async function grantTierCredits(accountId, tierId, paymentMeta = {}) {
  const account = await applyTierUpgrade(accountId, tierId);
