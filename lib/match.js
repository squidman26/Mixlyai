import { searchTrack, getProviderName } from "./music.js";
...
export async function matchRow(session, artist, title) {
  ...
  if (candidates.length === 0) {
    return { session: s, status: "not_found", track: null, score: 0 };
  }
...
export function notFoundReason(provider) {
  return `not found on ${getProviderName(provider)}`;
}
