...
async function appApi(path, options = {}) {
  ...
  if (!res.ok) {
    ...
    throw new Error(parseSpotifyError(res.status, body, path));
  }
...
export async function searchTrackApp(artist, title, limit = 5) {
  ...
  const data = await appApi(`/search?${params}`);
...
function parseSpotifyError(status, body, path) {
  let message = body;
  try {
  ...
  if (
    status === 403 &&
    /not registered|not approved|developer dashboard/i.test(message)
  ) {
    return "spotify_not_allowlisted";
  }

  return `Spotify API ${path} failed (${status}): ${message}`;
}
